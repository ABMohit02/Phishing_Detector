...
# Final decision (same logic as app.py)
if not resolves:
    result = "PHISHING"
    reason = "DNS failed"
elif age_days is None:
    result = "PHISHING" if legit < 0.75 else "LEGITIMATE"
    reason = "WHOIS unknown, falling back to ML"
elif age_days < 365:
    result = "PHISHING"
    reason = f"Domain only {age_days} days old"
else:
